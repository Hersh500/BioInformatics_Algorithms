#WORKS
#This program uses the same template of a dynamic programming matrix along with a bactrack matrix, with some modifications
import sys
...
#^ allows easy indexing into the blosum/pam scoring matrix
	...
	for i in range(1, len(dna1) + 1):
		for j in range(1, len(dna2) + 1):
			...
			if dna1[i-1] == dna2[j-1]:
				s[i][j] = min (s[i-1][j] + indel_penalty, s[i][j-1] + indel_penalty, s[i-1][j-1])
			else:
				s[i][j] = min (s[i-1][j] + indel_penalty, s[i][j-1] + indel_penalty, s[i-1][j-1] + 1)
...
#dna1 = "TNIYGLKERVPR"
#dna2 = "TNNMQTRMWCVLIAAPCHLW"
blosum_table = make_blosum()
#print(blosum_table)
backtrack, score = construct_backtrack(dna1, dna2,1, blosum_table)
print(score)
#print_matrix(backtrack)
#output_LCS(backtrack, dna1, dna2,  len(dna1), len(dna2), blosum_table)
#print( score (str1, str2, blosum_table) )
#print(str1)
#print(str2)
#print( cmpr_strings(str1, str2))
